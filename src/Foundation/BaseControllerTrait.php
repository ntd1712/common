 ...
 * @method AbstractBaseService|Contracts\IBaseService getService(string $name = null)
 */
trait BaseControllerTrait
{
    /**
    ...
    private function preparePagerParams(array $binds = [])
    {
        ...
        if (isset($binds['ItemCountPerPage'])) {
            ...
            } else if (($maxPerPage = $this->__getConfig()->get('app.max_items_per_page')) < $binds['ItemCountPerPage']) {
                $binds['ItemCountPerPage'] = $maxPerPage;
            }
        } else {
            $binds['ItemCountPerPage'] = $this->__getConfig()->get('app.items_per_page');
        }
