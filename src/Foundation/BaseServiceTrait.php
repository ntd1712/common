 ...
 * @method AbstractDoctrineRepository|Contracts\IBaseRepository getRepository(string $name = null)
 */
trait BaseServiceTrait
{
    ...
    public function prepareFilterParams($binds = [], $predicate = null)
    {
        ...
        if (is_array($binds)) {
            ...
            $searchable = $this->__getConfig()->get('app.min_search_chars') <= strlen($binds);
    ...
    public function filter($value, $checkDate = false)
    {
        ...
        if (false !== $checkDate && 0 !== preg_match(CHAOS_MATCH_DATE, $value, $matches)) {
            $filtered = date(
                $this->__getConfig()->get('framework.date_format'),
                is_bool($checkDate) ? strtotime($matches[0]) : strtotime($matches[0]) + $checkDate
            );
        } else {
            $filtered = StaticFilter::execute(
                $value, 'HtmlEntities', ['encoding' => $this->__getConfig()->get('framework.charset')]
            );
        }
